...
def test_get_minibatches():
    ...
    for dtype in ["cpu", "cuda:0"] if th.cuda.is_available() else ["cpu"]:
        ...
        minibatches = get_minibatches(
            states, probs, returns, advantages, n_minibatches, mb_size, device
        )

        assert (
            len(minibatches) == n_minibatches
        ), f"Expected {n_minibatches} minibatches, got {len(minibatches)}"

        for mb_states, mb_probs, mb_returns, mb_advantages in minibatches:
            assert (
                mb_states.shape[0] == mb_size
            ), f"Expected minibatch size {mb_size}, got {mb_states.shape[0]}"
            assert (
                mb_states.device == device
            ), f"Expected device {device}, got {mb_states.device}"
    ...
    for mb_states, _, _, _ in minibatches:
        for key in states.keys():
            ...
            assert (
                mb_states[key].shape[0] == mb_size
            ), f"Expected minibatch size {mb_size}, got {mb_states[key].shape[0]}"
