...
class ActorCritic(nn.Module):
    class Actor(nn.Module):
        def __init__(self, action_space:Union[ActionAttribute, Dict[str, ActionAttribute]], net_arch:list, activation_fnc=nn.ReLU, device='cuda'): 
            super(ActorCritic.Actor, self).__init__()
            self.device = device
            assert type(action_space.space) in [DiscreteType, MultiDiscreteType, BoxType]
            self.action_space = action_space
            if isinstance(action_space.space, BoxType):
                self._out_size = 2 * np.prod(action_space.space.shape)  # both mean and std dev
            else:
    ...
    """
        ...
        if actor_activation_fnc == None: actor_activation_fnc = activation_fnc
        if critic_activation_fnc == None: critic_activation_fnc = activation_fnc
        self.shared_net = generate_sequential(self.__shared_arch)
        self.__output_activation = activation_fnc()
        self.actor = ActorCritic.Actor(self.action_space, self.__actor_arch, actor_activation_fnc, self.device)
    ...
    def sample(self, obs):
        ...
        for num_actions in self.actor.action_outputs:
            ...
            action_probs, action_sample = sample_distrobution(actor_output[:, _action_idx:_action_idx+num_actions], self.action_space.space)
    ...
    def get_sample_and_values(self, obs):
        ...
        for num_actions in self.actor.action_outputs:
            # Sample action from the distribution and keep track of the probabilities
            action_log_probs, action_sample = sample_distrobution(actor_output[:, _action_idx:_action_idx+num_actions], self.action_space.space, self.action_space)
            sampled_actions.append(action_sample)
    ...
    def __get_net_arch(net_arch) -> Tuple[list, list, list]:
        ...
        actor_arch = []
        critic_arch = []
        for layer in net_arch: # Add each respective architecture to the correct Network List 
            if isinstance(layer, int):
                shared_arch.append(layer)
            elif isinstance(layer, dict):
                if 'vf' in layer: critic_arch = layer['vf']
                else: raise Exception("Must Specify a valid architecture (no Critic Network found.)")
                if 'pi' in layer: actor_arch = layer['pi']
                else: raise Exception("Must Specify a valid architecture (no Actor Network found.)")
