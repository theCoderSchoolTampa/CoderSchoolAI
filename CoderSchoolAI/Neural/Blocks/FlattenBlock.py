...
class FlattenBlock(Block):
    def __init__(self,
        ...
        """
        super(FlattenBlock, self).__init__(b_type=Block.Type.FLATTEN, activation_function=None, device=device)
        self.output_size = output_size
        
    
    def join_block(self, block: Block,  key:str = None):
                if key is None: self.forward_connections = block
                else: self.forward_connections[key] = block
                    
                
    
    def forward(self, x) -> th.Tensor:
        x = th.reshape(x, (-1, self.output_size))
