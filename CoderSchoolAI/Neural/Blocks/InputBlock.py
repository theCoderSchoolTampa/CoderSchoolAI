        ...
        if isinstance(in_attribute, dict):  # Multi-Attribute Networks
            ...
            for key, attr in in_attribute.items():
                if len(attr.space.shape) == 1:
                    ...
                    temp_dict_module[key] = dict(
                        dtype=Block.Type.LINEAR,
                        in_features=attr.space.shape[0],
                        shape=attr.space.shape,
                    )
                    flatten_size += attr.shape[0]
                elif len(attr.space.shape) > 1:
                    # This is a Convolutional Attribute
                    temp_dict_module[key] = (
                        dict(
                            dtype=Block.Type.CONV,
                            in_features=attr.space.shape[0],
                            shape=attr.space.shape,
                        )
                        if len(attr.space.shape) > 2
                        else dict(
                            dtype=nn.Conv2d,
                            in_channels=1,
                            shape=attr.space.shape.unsqueeze(),
                        )
                    )
            ...
            if len(in_attribute.space.shape) == 1:
                ...
                self.module = dict(
                    dtype=Block.Type.LINEAR,
                    in_features=attr.shape[0],
                    shape=in_attribute.space.shape,
                )
                ...
                self.module = (
                    dict(
                        dtype=Block.Type.CONV,
                        in_features=in_attribute.space.shape[0],
                        shape=in_attribute.space.shape,
                    )
                    if len(in_attribute.space.shape) > 2
                    else dict(
                        dtype=nn.Conv2d, in_channels=1, shape=in_attribute.space.shape
                    )
                )
