...
class OutputBlock(Block):
    def __init__(
        ...
        super(OutputBlock, self).__init__(
            b_type=Block.Type.OUTPUT, device=device, activation_function=activation
        )
    ...
    def forward(self, x) -> th.Tensor:
        ...
        if x.shape[-1] != self.input_size:
            raise Exception(
                f"OutputBlock.forward() expects a matrix of shape Batch Size x {self.input_size}, Provided Shape: {x.shape}"
            )
        x = self.module(x)
        return x
