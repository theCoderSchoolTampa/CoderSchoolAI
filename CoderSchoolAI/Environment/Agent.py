...
class Agent:
    ...
    def get_actions(self):
        ...
        raise NotImplementedError(
            "This method should be implemented in a subclass, not the Base Class Agent."
        )
...
class BasicReplayBuffer(ReplayBuffer):
    ...
    def store_memory(
        self, state, action, probs, vals, reward, done, next_state
    ) -> None:
...
class DictReplayBuffer(ReplayBuffer):
    ...
    def generate_batches(
        ...
        ns = (
            np.array(self.next_states)
            if not isinstance(self.next_states, dict)
            else {key: np.array(value) for key, value in self.next_states.items()}
        )
        return (
            s,
            a,
    ...
    def clear_memory(self) -> None:
        self.states = (
            {key: [] for key in self.dict_keys} if self.dict_keys is not None else []
        )
        self.actions = (
            {key: [] for key in self.act_keys} if self.act_keys is not None else []
        )
        self.probs = []
        self.vals = []
        self.rewards = []
