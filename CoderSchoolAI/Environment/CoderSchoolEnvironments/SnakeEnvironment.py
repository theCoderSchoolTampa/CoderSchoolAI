...
class SnakeAgent(Agent):
    ...
    def __init__(
        ...
        policy_kwargs=dict(
            alpha=0.5, gamma=0.9, epsilon=0.9, epsilon_decay=0.995, stop_epsilon=0.01
        ),
    ):
        """ """
    ...
    def _move_snake(self, action: "SnakeAgent.SnakeAction") -> "SnakeAgent.SnakeAction":
        ...
        _head = self.body[-1]
        if (
            action == SnakeAgent.SnakeAction.NOACTION
            or np.dot(self.DIRECTIONS[action], self.DIRECTIONS[self.last_action]) < 0
        ):
            feedback = -0.2 if action != SnakeAgent.SnakeAction.NOACTION else -0.05
            action = self.last_action
        _new_head = (
            _head[0] + self.DIRECTIONS[action][0],
            _head[1] + self.DIRECTIONS[action][1],
        )
    ...
    def get_q_table_state(self, state) -> Tuple:
        ...
        l = (
            self.DIRECTIONS[SnakeAgent.SnakeAction.LEFT][0] + s_x,
            self.DIRECTIONS[SnakeAgent.SnakeAction.LEFT][1] + s_y,
        )
        r = (
            self.DIRECTIONS[SnakeAgent.SnakeAction.RIGHT][0] + s_x,
            self.DIRECTIONS[SnakeAgent.SnakeAction.RIGHT][1] + s_y,
        )
        u = (
            self.DIRECTIONS[SnakeAgent.SnakeAction.UP][0] + s_x,
            self.DIRECTIONS[SnakeAgent.SnakeAction.UP][1] + s_y,
        )
        d = (
            self.DIRECTIONS[SnakeAgent.SnakeAction.DOWN][0] + s_x,
            self.DIRECTIONS[SnakeAgent.SnakeAction.DOWN][1] + s_y,
        )
        ...
        def find_path_astar(start, end, snake_body):
            ...
            while frontier.size() > 0:
                ...
                for action in list(SnakeAgent.SnakeAction):
                    ...
                    if (
                        new_pos not in c_from
                        or new_cost < min_cost[new_pos]
                        and not any(
                            [
                                new_pos[0] == s[0] and new_pos[1] == s[1]
                                for s in snake_body
                            ]
                        )
                    ):
...
class SnakeEnv(Shell):
    ...
    def __init__(
        ...
        super().__init__(
            target_fps,
            is_user_control,
            resolution=(height * cell_size, width * cell_size),
            verbose=verbose,
            environment_name="Snake-Env",
        ...
        self.snake_agent = SnakeAgent(
            self.is_user_control,
            snake_is_q_table,
            snake_is_search_enabled,
            policy_kwargs,
        )
        ...
        apple_res = pkg_resources.resource_filename(
            "CoderSchoolAI", "Assets/Snake/Apple.png"
        )
        self.apple_asset = pygame.image.load(apple_res).convert()
        self.apple_asset = pygame.transform.scale(
            self.apple_asset, (self.cell_size, self.cell_size)
        )
    ...
    def reset(self, attributes=None) -> Tuple[
        ...
        if self.verbose:
            (
                print("Resetting Apple Position.")
                if self._soft_reset
    ...
    def update_env(self):
        ...
        if self.snake_agent.is_q_table:
            self.snake_agent.qlearning.update_q_table(
                self.snake_agent.get_q_table_state(prev_state),
                int(action),
                reward,
                self.snake_agent.get_q_table_state(new_state),
            )
    ...
    def render_env(self):
        ...
        if not self.console_only:
            ...
            rect = pygame.Rect(
                head_position[0] * self.cell_size,
                head_position[1] * self.cell_size,
                self.cell_size,
                self.cell_size,
            )
            ...
            for i in range(self.width):  # Assuming grid_size is the number of cells
                pygame.draw.line(
                    ...
                    (255, 255, 255),
                    (i * self.cell_size, 0),
                    (i * self.cell_size, self.width * self.cell_size),
                )  # Vertical lines
            for i in range(self.width):  # Assuming grid_size is the number of cells
                pygame.draw.line(
                    self.screen,
                    (255, 255, 255),
                    (0, i * self.cell_size),
                    (self.height * self.cell_size, i * self.cell_size),
                )  # Horizontal lines

            # Draw the score
            score_text = self.font.render(
                f"Score: {self._apples_consumed}", True, (225, 220, 128)
            )
            self.screen.blit(
                score_text,
                (self.width * self.cell_size - score_text.get_width() - 5, 5),
    ...
    def update_agent_data(self):
        ...
        self.game_state[self.snake_agent.body[-1]] = int(
            SnakeEnv.GameObjects.HEAD
        )  # Snake head
    ...
    def consumed_apple(
        ...
        """
        return (
            self.apple_position[0] == self.snake_agent.body[-1][0]
            and self.apple_position[1] == self.snake_agent.body[-1][1]
        )

    def _is_snake_in_bounds(
        self,
    ):
        """
        ...
        """
        head_position = self.snake_agent.body[-1]
        return (0 <= head_position[0] < self.width) and (
            0 <= head_position[1] < self.height
        )
