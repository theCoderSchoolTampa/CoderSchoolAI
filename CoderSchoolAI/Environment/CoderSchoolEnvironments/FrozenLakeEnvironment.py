...
class FrozenLakeEnv(Shell):
    ...
    def __init__(self, height=4, width=4, verbose=False):
        super().__init__(console_only=True, environment_name="FrozenLake", verbose=verbose,)  # Initialize as per your Shell API
        self.height = height
        self.width = width
    ...
    def reset(self, attributes=None):
        ...
        self.grid[1][1] = self.grid[2][2] = self.GameObjects.HOLE  # Adding holes for example
    ...
    def __update_game_state_callback(self):
        self['game_state'].data = self.grid.copy().reshape(1, self.height, self.width) * (0.333)
